...
class vgg16(Network):
  ...
  def _image_to_head(self, is_training, reuse=None):
    ...
    print("这里是这里是这里是特征图的shape：",self._layers['head'].shape)
  ...
  def fix_variables(self, sess, pretrained_model):
    ...
    with tf.variable_scope('Fix_VGG16') as scope:
      with tf.device("/cpu:0"):
        ...
        # fix RGB to BGR
        fc6_conv = tf.get_variable("fc6_conv", [7, 7, 512, 4096], trainable=False) # 原数据为[7, 7, 512, 4096]
