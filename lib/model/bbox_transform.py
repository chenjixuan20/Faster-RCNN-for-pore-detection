...
def bbox_transform(ex_rois, gt_rois):  # ex/gt分别表示什么?？
  # ex示示计算给出的boxes的坐标位置
  # gt表示的是真实的坐️标的位置
  ex_widths = ex_rois[:, 2] - ex_rois[:, 0] + 1.0   # x坐标之差，也就是宽度
  ex_heights = ex_rois[:, 3] - ex_rois[:, 1] + 1.0   # y坐标之差，也就是高度

  ex_ctr_x = ex_rois[:, 0] + 0.5 * ex_widths  # 中心坐标:x轴
  ex_ctr_y = ex_rois[:, 1] + 0.5 * ex_heights  # 中心坐标:y轴

  gt_widths = gt_rois[:, 2] - gt_rois[:, 0] + 1.0  # 宽度
  gt_heights = gt_rois[:, 3] - gt_rois[:, 1] + 1.0 # 高度
  gt_ctr_x = gt_rois[:, 0] + 0.5 * gt_widths  # 中心坐标
